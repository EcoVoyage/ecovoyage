...
def _(mo):
    ...
    - **Shortbread map** — the de-facto general-purpose OSM vector-
      ...
      served by `versatiles serve` on port 28090 (alongside martin
      on 23000), styled with the `colorful()` template from
      `@versatiles/style`.
    ...
    port 28090. The notebook also ships a **versatiles convert
    round-trip demo cell** exercising the `versatiles` CLI's
    symmetric PMTiles ↔ .versatiles container conversion. Use
    the PMTiles Viewer at `http://127.0.0.1:28001/` or the
    VersaTiles Frontend at `http://127.0.0.1:28002/` to inspect
            ...
            def download_pbf() -> str:
                ...
                if out.exists():
                    try:
                        ...
                        # HEAD endpoint flaky; let the GET loop decide.
                        pass
...
def _(dag_run_states, mo, os):
    ...
    # `versatiles serve` on port 8090 (host 28090), rendered with
    # the `colorful()` style from @versatiles/style. The bundle is
    # served as a static asset by the versatiles-frontend layer's
    # http.server at /style/, so the iframe can <script src=...> it
    # without a CDN dependency.
    #
    # Compared to the four pipelines above (all on martin at 23000),
    # this cell exercises the parallel infrastructure: a different
    # OSM schema (shortbread vs the others' raw quackosm output),
    # a different tile server (versatiles vs martin), and a
    # different style generator (@versatiles/style vs the inline
    # MapLibre style JSON in build_pipeline_maplibre_html).
    assert dag_run_states["notebook_osm_shortbread_pipeline"] == "success"
    _versatiles_url = os.environ.get(
        "VERSATILES_PUBLIC_URL", "http://127.0.0.1:28090",
    )
    _style_url = os.environ.get(
        "VERSATILES_STYLE_PUBLIC_URL",
        "http://127.0.0.1:28002/style",
    )
    ...
    # Local-asset base — the versatiles-frontend layer's http.server
    # re-exports versatiles-style / fonts / styler at /style/, /fonts/,
    # /styler/ paths on port 28002. The iframe's <script src> + style
    # glyphs URL pattern all resolve against this single origin.
    _assets = os.environ.get(
        "VERSATILES_ASSETS_PUBLIC_URL", "http://127.0.0.1:28002",
    )
